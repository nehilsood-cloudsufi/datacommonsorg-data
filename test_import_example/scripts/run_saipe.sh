#!/bin/bash
# Dataset: Census SAIPE (Small Area Income and Poverty Estimates)
# Description: Run Claude agentic import workflow on US Census poverty data
...
DATASET_NAME="saipe"
SOURCE_DIR="$ROOT_DIR/statvar_imports/census_v2/saipe"
OUTPUT_DIR="$ROOT_DIR/test_import_example/$DATASET_NAME"

echo "=========================================="
echo "Running Claude Import: Census SAIPE"
echo "=========================================="
...
cp "$SOURCE_DIR/test_data/input_file/saipe_test_input.csv" "$OUTPUT_DIR/input/"
cp "$SOURCE_DIR/saipe_pvmap.csv" "$OUTPUT_DIR/input/expected_pvmap.csv"
cp "$SOURCE_DIR/test_data/output_files/census_saipe_output.csv" "$OUTPUT_DIR/input/expected_output.csv"
cp "$SOURCE_DIR/test_data/output_files/census_saipe_output.tmcf" "$OUTPUT_DIR/input/expected_output.tmcf"
...
python "$ROOT_DIR/tools/agentic_import/pvmap_generator.py" \
  --input_data="input/saipe_test_input.csv" \
